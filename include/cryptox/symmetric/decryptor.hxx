...
namespace cryptox {
	...
	class decryptor : public basic_endec<Cipher, EVP_DecryptInit_ex, EVP_DecryptUpdate, EVP_DecryptFinal_ex> {
		typedef basic_endec<Cipher, EVP_DecryptInit_ex, EVP_DecryptUpdate, EVP_DecryptFinal_ex> base_type;
	public:
		decryptor(
			const char* key,  const size_t  key_size,
			const char* salt, const size_t salt_size,
			const char* iv,   const size_t   iv_size = decryptor<Cipher>::iv_size(),
		...
		decryptor(
			...
			const char* iv = 0, const size_t iv_size = decryptor<Cipher>::iv_size(),
			const size_t key_rounds = 64*1024
		) : base_type(
